...
export default class DecryptNote extends LightningElement {
	...
	get objectName(){
		return SECURE_NOTE.objectApiName;
	}
	...
	getNoteObjectInfo({error,data}){
		if(!error && data){
			this.customFieldsArray = Object.values(data.fields).filter(field=>(field.apiName.endsWith('__c') && field.reference===true)).map(field=>this.objectName+field.apiName);
		}else if(error)
